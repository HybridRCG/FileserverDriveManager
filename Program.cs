...
namespace FileserverDriveManager
{
    ...
    {
        private const string APP_VERSION = "v3.5";
        ...
        private void CheckAndAutoConnect()
        {
            ...
            {
                Log($"{selectedVPNProvider} IP not found - launching VPN...");
                LaunchVPN();
                ...
                for (int i = 0; i < 10; i++)
                {
                    ...
                    if (!string.IsNullOrEmpty(vpnIP) && !vpnIP.Contains("Not Connected"))
                    {
                        Log($"{selectedVPNProvider} connected: {vpnIP}");
                        break;
                    }
                }
            }
        ...
        private void SettingsButton_Click(object sender, EventArgs e)
        {
            ...
            installNetBirdButton.Click += (s, ev) =>
            {
                try
                {
                    Process.Start(new ProcessStartInfo
                    {
                        FileName = "https://netbird.io/docs/getting-started/installation",
                        UseShellExecute = true
                    });
                }
                catch (Exception ex)
